...
public class HistoryMain {
    public static void main(String[] args){
    	...
    	if (args[0].equals("checkAll"))
    		...
    		String finds[] = {"InvocationHandler", "http://www.cnblogs.com/xiaoluo501395377/p/3383130.html"};
    		int before = 10;
    		int after = 5;
    		if (args[0].length() > 1)
    			finds = args[0].trim().split(",");
    		if (args.length > 1)
    			before = Integer.parseInt(args[1]);
    		if (args.length > 2)
    			after = Integer.parseInt(args[2]);
    		new HsitoryManager().checkSequence(folderPath, finds, before, after);
    	}
    }
}